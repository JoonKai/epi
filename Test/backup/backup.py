...
class SusceptorViewer(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # ★ 추가: 현재 표시 모드
        self._view_mode: str = self.MODE_CIRCLE
    ...
    def setViewMode(self, mode: str):
        if mode not in (self.MODE_CIRCLE, self.MODE_HORIZONTAL):
            mode = self.MODE_CIRCLE
    ...
    # ★ 추가: Horizontal 배치 좌표 계산 (wafer 리스트 + hit-test도 동일하게 쓰려고)
    def _calc_layout_horizontal(self) -> List[Tuple[int, int, float, float, float, float]]:
        """
        반환은 build_wafers_with_ring_index와 동일 tuple이지만,
        wx,wy 값을 '가로 일자' 배치에 맞춘 world(인치) 좌표로 다시 넣어준다.
        """
        if not self._spec:
            return []

        wr = wafer_radius_inch(self._spec)

        # 링 순서대로 wafer를 일렬로 나열
        base = build_wafers_with_ring_index(self._spec)

        # 간격: wafer 지름 + gap
        pitch = (wr * 2.0) + GAP_INCH
        ...
        out = []
        for idx, (ring_idx, no, _wx, _wy, _wr, _flat_ang) in enumerate(base):
            x = start_x + idx * pitch

            # flat은 링의 IN/OUT에 따라 "위/아래"로 표시(가로모드에서 직관적으로)
            # IN  -> 위쪽(270deg, GDI 기준 위)
            # OUT -> 아래쪽(90deg, GDI 기준 아래)
            ring = self._spec.rings[ring_idx] if 0 <= ring_idx < len(self._spec.rings) else None
            if ring and ring.flat_direction == "OUT":
                flat_dir_ang = 90.0
            else:
                flat_dir_ang = 270.0

            out.append((ring_idx, no, x, y, _wr, flat_dir_ang))
        return out
    ...
    def paintEvent(self, e):
        ...
        if self._view_mode == self.MODE_HORIZONTAL:
            self._paint_horizontal(p)
        else:
            self._paint_circle(p)

    def _paint_circle(self, p: QPainter):
        cx, cy, world_to_px, r_to_px = self._calc_view_circle()
        ...
        for ring_idx, no, wx, wy, wr, flat_ang in wafers:
            ...
            # flat red arc (기존 로직 유지)
            gdi_dir = SUSCEPTOR_BASIS_ANGLE_DEG + flat_ang
            g = 10.0
            p.setPen(self._pen_red2)
            p.drawArc(rect, gdi_angle_to_qt_start16(gdi_dir - g), int((-g * 2.0) * 16))

    def _paint_horizontal(self, p: QPainter):
        """
        Horizontal 모드:
        - Susceptor는 원 대신 '가로 기준선' + (선택사항) 외곽 사각형 느낌
        - Wafer는 링 순서대로 좌->우 일렬 배치
        - 링 선택은 동일(wafer 클릭시 해당 ring_idx 선택)
        """
        if not self._spec:
            return
        ...
        wafers = self._calc_layout_horizontal()
        if not wafers:
            return

        # 화면 스케일: 전체 길이가 화면에 맞도록
        # world(x)는 inch 단위
        min_x = min(w[2] for w in wafers)
        max_x = max(w[2] for w in wafers)
        wr = wafer_radius_inch(self._spec)
        world_w = (max_x - min_x) + (wr * 2.0) + GAP_INCH * 2.0
        world_h = (wr * 2.0) * 4.0  # 여유

        sx = (W - 2 * SUSCEPTOR_SIDE_GAP_PX) / world_w if world_w > 0 else 1.0
        sy = (H - 2 * SUSCEPTOR_SIDE_GAP_PX) / world_h if world_h > 0 else 1.0
        scale = min(sx, sy)
        ...
        # (선택) susceptor 영역 느낌의 얇은 라운드 사각형
        pad = r_to_px(wr * 2.0)
        rect_outer = QRectF(
            SUSCEPTOR_SIDE_GAP_PX,
            cy - pad,
            W - 2 * SUSCEPTOR_SIDE_GAP_PX,
            pad * 2,
        )
        p.setPen(QPen(QColor("#333"), 1))
        p.drawRoundedRect(rect_outer, 8, 8)

        # wafers draw
        for ring_idx, no, wx, wy, wr_in, flat_dir_gdi in wafers:
            cpt = world_to_px(wx, wy)
            rp = r_to_px(wr_in)
            rect = QRectF(cpt.x() - rp, cpt.y() - rp, rp * 2, rp * 2)
            ...
            # flat red arc (가로모드에서는 IN=위, OUT=아래로 표시)
            g = 12.0
            p.setPen(self._pen_red2)
            p.drawArc(rect, gdi_angle_to_qt_start16(flat_dir_gdi - g), int((-g * 2.0) * 16))
    ...
    def mousePressEvent(self, e):
        ...
        if self._view_mode == self.MODE_HORIZONTAL:
            ...
            hit_ring = -1
            for ring_idx, no, wx, wy, wr_in, _flat in wafers:
                cpt = world_to_px(wx, wy)
                rp = r_to_px(wr_in)
                dx = mx - cpt.x()
                dy = my - cpt.y()
            ...
            if hit_ring != -1:
                ...
                self.ringSelected.emit(hit_ring)
                self.update()
        ...
        _, _, world_to_px, r_to_px = self._calc_view_circle()
        wafers = build_wafers_with_ring_index(self._spec)
        if not wafers:
            return
...
# ------------------------------------------------------------
# Main window / Tabs
#  - 핵심: 하나의 splitter(좌측패널+viewer)를 MultiRing/Horizontal 탭 사이에서 '이동'시키면서
#          viewer.setViewMode만 바꿔서 같은 UI를 그대로 쓰게 함.
# ------------------------------------------------------------
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # ★ 핵심: viewer/panel/splitter는 단 하나만 만들고, 탭에 따라 옮겨 붙인다.
        self.viewer = SusceptorViewer()
        self.viewer.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)

        self.left = MultiRingPanel(self.viewer)
        self.left.setMinimumWidth(340)
        ...
        # 탭 페이지들
        self.page_multiring = QWidget()
        self.page_horizontal = QWidget()

        self.lay_multiring = QHBoxLayout(self.page_multiring)
        self.lay_multiring.setContentsMargins(6, 6, 6, 6)
        self.lay_multiring.addWidget(self.splitter)

        self.lay_horizontal = QHBoxLayout(self.page_horizontal)
        self.lay_horizontal.setContentsMargins(6, 6, 6, 6)
        # 여기엔 처음엔 아무것도 안 넣음(탭 전환 시 splitter 이동)

        self.tabs.addTab(self.page_multiring, "MultiRing")
        self.tabs.addTab(self._placeholder("Satellites"), "Satellites")
        self.tabs.addTab(self._placeholder("Hexagons"), "Hexagons")
        self.tabs.addTab(self._placeholder("Free"), "Free")
        # ✅ Free 뒤에 Horizontal 탭 추가
        self.tabs.addTab(self.page_horizontal, "Horizontal")
    ...
    def _move_splitter_to(self, target_layout: QHBoxLayout):
        # splitter가 이미 다른 레이아웃에 붙어있으면 떼고, target에 붙임
        parent = self.splitter.parentWidget()
        if parent is not None:
            pl = parent.layout()
            if pl is not None:
                pl.removeWidget(self.splitter)
        target_layout.addWidget(self.splitter)

    def _on_tab_changed(self, idx: int):
        name = self.tabs.tabText(idx)
        if name == "Horizontal":
            self.viewer.setViewMode(SusceptorViewer.MODE_HORIZONTAL)
            self._move_splitter_to(self.lay_horizontal)
        elif name == "MultiRing":
            self.viewer.setViewMode(SusceptorViewer.MODE_CIRCLE)
            self._move_splitter_to(self.lay_multiring)
        # 다른 탭은 placeholder니까 아무것도 안 함
